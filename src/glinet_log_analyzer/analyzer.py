...
SEVERITY_PATTERNS = {
    ...
    "warning": re.compile(r"\b(?:warning|warn|degraded|retry)\b", re.IGNORECASE),
    "info": re.compile(r"\b(?:info|notice|connected|started|ready|up)\b", re.IGNORECASE),
...
CATEGORY_PATTERNS = {
    ...
    "cellular": re.compile(r"\b(?:modem|lte|5g|wwan|mbim|qmi|sim card|signal)\b", re.IGNORECASE),
...
SIGNAL_PATTERNS = {
    "wan_up": re.compile(r"\b(?:wan link is up|interface ['\"]?wan['\"]?.*is up|udhcpc: lease of)\b", re.IGNORECASE),
    "wan_down": re.compile(r"\b(?:lost the connection|wan link is down|carrier lost|pppoe.*disconnected)\b", re.IGNORECASE),
    "dhcp_lease": re.compile(r"\b(?:lease of \d{1,3}(?:\.\d{1,3}){3}|dhcpack|dhcpoffer)\b", re.IGNORECASE),
    "wifi_client_join": re.compile(r"\b(?:AP-STA-CONNECTED|associated)\b", re.IGNORECASE),
    "wifi_client_leave": re.compile(r"\b(?:AP-STA-DISCONNECTED|disassociated|deauthenticated)\b", re.IGNORECASE),
    ...
    "modem_event": re.compile(r"\b(?:modem|sim card|lte|5g|wwan|qmi|mbim)\b", re.IGNORECASE),
    ...
    "cell_signal": re.compile(r"\b(?:rsrp|rsrq|sinr|rssi|signal quality)\b", re.IGNORECASE),
...
def analyze_documents(documents: list[LogDocument]) -> AnalysisResult:
    ...
    for document in documents:
        ...
        for line_number, raw_line in enumerate(document.text.splitlines(), start=1):
            ...
            severity = _detect_severity(remainder)
